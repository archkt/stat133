...
```{r}
# your code
states_first_five = head(states, 5)
states_char = paste(states_first_five, num_chars[1:5], sep = " = ")
states_char
```
...
```{r}
# your code
paste(states_first_five, collapse = '')
```
...
for(i in 1:length(char_freqs)){
  states_names[i] = paste0(row.names(char_freqs)[i], "-chars")
  name = ""
  if (strtoi(row.names(char_freqs)[i]) %% 2 == 0) {
    state_name = toupper(paste(states[num_chars == row.names(char_freqs)[i]], collapse = ", "))
    
  } else {
    state_name = tolower(paste(states[num_chars == row.names(char_freqs)[i]], collapse = ", "))
  }
  states_list[i] = state_name
}

names(states_list) = states_names
states_list
```
...
file_with_paste = c()
for (i in 1:10) {
  file_with_paste[i] = paste("file", i,".csv",sep = "")
}
file_with_paste
```
...
file_with_paste0 = c()
for (i in 1:10) {
  file_with_paste0[i] = paste0("file", i,".csv")
}
file_with_paste0
```
...
```{r}
# your code
gsub(pattern = 'file', replacement =  'dataset', x= file_with_paste)
```
